...
import com.marketplace.order.service.OrderValidationService;
...
import com.marketplace.product.service.ProductsValidationResponse;
...
public class UserOrderController {
    ...
    public String getOrderById(
        ...
        ProductsValidationResponse productsValidationResponse = productManagerService.validateProducts(order.getProductIds());

        model.addAttribute("isPayable", !productsValidationResponse.hasInvalidProduct());
        model.addAttribute("products", simpleProductMapper.mapProductsToProductResponseDtos(productsValidationResponse.products()));
    ...
    public String getUserOrder(
        ...
        orderByOwnerIdAndStatus.ifPresent(order -> {
            ProductsValidationResponse productsValidationResponse = productManagerService.validateProducts(order.getProductIds());
            model.addAttribute("isPayable", !productsValidationResponse.hasInvalidProduct());
            model.addAttribute("orderProducts", simpleProductMapper.mapProductsToProductResponseDtos(productsValidationResponse.products()));
            model.addAttribute("totalSum", orderManagerService.calculateTotalSum(productsValidationResponse.products()));
